...
# Starts a named pgvector container with PostgreSQL 16, exposes it on host port 6024, and sets up the langchain DB with credentials.
...
CREATE TABLE documents (
    ...
    embedding vector(384) NOT NULL,  -- 384 dims: sentence-transformers/all-MiniLM-L6-v2 (free, local)
...
SELECT id, content, source_file, page_number, chunk_index, department, access_level, created_at 
FROM documents 
LIMIT 5;
...
# Creates an HNSW index on the `embedding` column using cosine similarity;
# builds a multi-layer graph where each node connects to m=16 nearest neighbors,
# ef_construction=64 controls candidate list size during build — higher = better quality but slower indexing.
...
# Creates an IVFFlat index on the `embedding` column using cosine similarity; 
# partitions vectors into 25 clusters (lists) for faster approximate nearest-neighbor search 
# skips full scan by only searching the closest cluster(s) at query time.
